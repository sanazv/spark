...
#inputPath = os.path.join('millionsong.txt') #sanaz c
#fileName = os.path.join(baseDir, inputPath) #sanaz c
...
##sanaz fileName = '/home/vagrant/' + os.path.join(baseDir, inputPath)
#print fileName
numPartitions = 2
...
def parsePoint(line):
    ...
    feature=str(line).split(',')[1:]
    #return line.split(',') #LabeledPoint(label,feature)
...
def calcRMSE(labelsAndPreds):
    ...
    sumerr=labelsAndPreds.map(lambda x:squaredError(x[0],x[1])).sum()
    
    n=labelsAndPreds.count()
    return (sumerr/float(n))**0.5
#print squaredError(2, 2.5)
...
def gradientSummand(weights, lp):
    ...
    #print DenseVector.dot(w,x)
    return (DenseVector.dot(w,x)-y)*x
...
def getLabeledPrediction(weights, observation):
    ...
    w=DenseVector(weights)
    #print type(x),type(w)
    pred=DenseVector.dot(w,x)
    #Note that we can predict by computing the dot product between weight
    #and an observation's features.
    return (label,pred)
#print getLabeledPrediction(np.array([1.0, 1.5]),LabeledPoint(2, np.array([1.0, .5])))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        #labelsAndPredsTrain = trainData.map(lambda x : getLabeledPrediction(w,trainData))
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)
        #print errorTrain
        gradient = trainData.map(lambda x:gradientSummand(DenseVector(w),x)).reduce(lambda x,y:x+y)
        # Update the weights
        alpha_i = alpha / (n * np.sqrt(i+1))
        #print i,alpha_i, w, gradient
...
#ww,ee=  linregGradientDescent(parsedTestData, 2)
#print ww,ee
...
#print nVal, D, nTrain
#allexampleData = (sc
#               .parallelize(parsedTrainData.take(nTrain))
#               .map(lambda lp: LabeledPoint(lp.label, lp.features[0:D])))

#print type(parsedTrainData)
#print parsedTrainData.take(1)
#AlltrainData = (sc
#               .parallelize(parsedTrainData.take(nTrain))  #nTrain
#               .map(lambda lp: LabeledPoint(lp.label, lp.features[0:D])))
weightsLR0, errorTrainLR0 = linregGradientDescent(parsedTrainData, numIters)


#print 'hello', len(weightsLR0), len(errorTrainLR0)
#print weightsLR0,errorTrainLR0
#print "hello2"

#labelsAndPreds = parsedValData.map(lambda x: (x.label, errorTrainLR0[?? what the predicion now is]))
labelsAndPreds = parsedValData.map(lambda x: (getLabeledPrediction(weightsLR0,x)))
#print type(labelsAndPreds)
#print "hello3"
rmseValLR0 = calcRMSE(labelsAndPreds)
print rmseValLR0,weightsLR0,rmseValBase
...
# TODO: Replace <FILL IN> with appropriate code
#firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations=10,initialWeights=[0,0,0])
firstModel =LinearRegressionWithSGD.train(parsedTrainData,iterations=numIters, step=alpha,
regParam=reg, miniBatchFraction=miniBatchFrac,intercept=useIntercept,regType = regType)
...
print rmseValLRGrid # take this out later after 3d is fixed
print ('Validation RMSE:\n\tBaseline = {0:.3f}\n\tLR0 = {1:.3f}\n\tLR1 = {2:.3f}\n' +
...
def twoWayInteractions(lp):
    ...
    """
    f=lp.features
    new=list(itertools.product(f,f))
    ...
    for i in range(len(new)):
        ...
        newgood.append(nn)
    #print newgood
    newlist=np.hstack((f,newgood))
    #print newlist
    return LabeledPoint(lp.label,newlist)
