...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    revid=IDandRatingsTuple[0]
    ratings=IDandRatingsTuple[1]
    revcount=len(ratings)
    revave=sum(ratings)/float(revcount)
    return (revid,(revcount, revave))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda (y,x): getCountsAndAverages((y,x))).sortBy(lambda (y,x): y)
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda (x,y): (y[1][1],y[0],y[1][0]))).sortBy(lambda (x,y,z):x)
...
def computeError(predictedRDD, actualRDD):
    ...
    #print predictedReformattedRDD.take(4)
    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda (x,y,z):((x,y),z))
    #print actualReformattedRDD.take(4)
...
trainingAvgRating = (trainingRDD.map(lambda (x,y,z): z).sum())/float(trainingRDD.count())
...
print '(average rating, movie name, number of reviews, movieID)'
for ratingsTuple in myRDD.take(120):
...
myRatedMovies = [(myUserID, 318, 5),(myUserID, 527, 3),(myUserID, 50, 4.5),(myUserID, 260, 5),
     (myUserID, 593, 4),(myUserID, 296, 4.5),(myUserID, 2858, 3.5),(myUserID, 1225, 3.5),
     (myUserID, 541, 4.5),(myUserID, 1240, 5),(myUserID, 1089, 4),(myUserID, 110, 4.5),
     (myUserID, 1210, 5),(myUserID, 1270, 5),(myUserID, 3578, 4.5),(myUserID, 1961, 4.5),
     (myUserID, 1206, 4.5),(myUserID, 2396, 4),(myUserID, 480, 4),(myUserID, 2959, 5),
...
myids=[y for (x, y, z) in myRatedMovies]
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda (x,y): y not in myids)).map(lambda (x,y): (0,x))
#print myids
#print myUnratedMoviesRDD.take(20)
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)).map(lambda (x,y): (y[0][0],y[1],y[0][1])).filter(lambda (x,y,z): z>75)
