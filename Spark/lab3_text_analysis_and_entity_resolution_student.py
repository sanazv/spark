...
def tokenize(string):
    ...
    """
    s= re.split(split_regex,string.lower())
    return filter(lambda x: len(x)>0 and x not in stopwords,s)
    #return <FILL IN>
...
def findBiggestRecord(vendorRDD):
    ...
    return sorted(vendorRDD.map(lambda (x,y): (x,y)).reduceByKey(lambda x,y: x+y).collect(),key=lambda word:-1*len(word[1]))
...
#return {k: float(v)/len(tokens) for k, v in tdic.items()}
...
def idfs(corpus):
    ...
    """
    N = len(corpus.map(lambda x: x[0]).collect()) 
    #print N=400 correct
...
# in how many documents it appeared, don't care how many times in each
    return (tokenSumPairTuple.map(lambda (x,y): (x, float(N)/float(y)))) #N is correct

idfsSmall = idfs(amazonRecToToken.union(googleRecToToken))
uniqueTokenCount = idfsSmall.count()

#print (idfsSmall.collect())
...
def tfidf(tokens, idfs):
    ...
    dict1=tfs
    dict2=idfs
    tfIdfDict = {k : v * dict2[k] for k, v in dict1.items() if k in dict2}
    #tfs.map(lambda (x,y): (x,y*idfs)).collect()
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    return sum({k : b[k] * a[k] for k, v in a.items() if k in b}.values())
...
def norm(a):
    ...
    """
    return (sum({k :  a[k]**2 for k in a}.values()))**(0.5)
...
def cosineSimilarity(string1, string2, idfsDictionary):
    ...
    """
    w1 = tfidf(tokenize(string1),idfsSmallWeights)
    w2 = tfidf(tokenize(string2),idfsSmallWeights)
    return cossim(w1, w2)

cossimAdobe = cosineSimilarity('Adobe Photoshop',
                               'Adobe Illustrator',
                               idfsSmallWeights)
#tfidf(recb000hkgj8k, idfsSmallWeights)
print cossimAdobe
...
def similarBroadcast(amazonID, googleURL):
    ...
    #return similaritiesBroadcast.filter(lambda record: (record[0] == googleURL and record[1] == amazonID)).map(lambda x: (x[0]+" "+x[1],x[2])).collect()
...
avgSimDups = float(trueDupsRDD.map(lambda (x,y):y).sum())/float(trueDupsCount)

nonDupsRDD = (sims
             .subtractByKey(goldStandard))
avgSimNon = float(nonDupsRDD.map(lambda (x,y):y).sum())/float(nonDupsRDD.count())
...
#amazonFullRecToToken.map(lambda x: (x[0],(tfidf(x[1],idfs(amazonFullRecToToken).collectAsMap()))))
...
def invert(record):
    ...
    for i in range(len(record[1].keys())):
        pairs.append((record[1].keys()[i],record[0]))
...
commonTokens = amazonInvPairsRDD.join(googleInvPairsRDD).map(lambda x: swap(x)).groupByKey().cache()
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1] 
    # for individual test add [0] to the begining of all the recs above
    s=0.
    for t in tokens:
    
        s=(amazonWeightsBroadcast.value[amazonRec][t]*googleWeightsBroadcast.value[googleRec][t])+s
