...
APACHE_ACCESS_LOG_PATTERN = APACHE_ACCESS_LOG_PATTERN = '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+) (\S+)\s*(\S*)\s*" (\d{3}) (\S+)'
#bad one '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+) (\S+)\s*(\S*)" (\d{3}) (\S+)'
...
not200 = access_logs.filter(lambda log: log.response_code > 299)
...
uniqueHosts = hosts.filter(lambda s: s[1] >= 1)

uniqueHostCount = uniqueHosts.reduceByKey(lambda a, b: a+b).count()
...
# makes a day and list of hosts

dayHostCount = dayGroupedHosts.map(lambda (day,host): (day,len(set(list(host)))))
#counts the hosts for each day

dailyHosts = dayHostCount.sortByKey().cache()
# sort by days and cache it for later

dailyHostsList = dailyHosts.take(30)

#print the top 30 which is one month worth
...
avgDailyReqPerHost = (sortedByDay.join(dailyHosts).map(lambda (a,b): (a, b[0]/b[1])).sortByKey().cache())
...
badUniqueEndpoints = badEndpoints.reduceByKey(lambda a,b: a+b).filter(lambda s: s[1]==1)
