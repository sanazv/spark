...
def wordCount(wordListRDD):
    ...
    outputRDD=wordListRDD.map(lambda x: (x,1)).reduceByKey(lambda x,y: x+y)
    return outputRDD
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = shakeWordsRDD.map(lambda x: (x,1)).reduceByKey(lambda x,y: x+y).takeOrdered(15, lambda(w,c):-c)
